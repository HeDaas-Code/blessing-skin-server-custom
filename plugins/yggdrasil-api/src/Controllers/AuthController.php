...
use Yggdrasil\Services\OfficialRouter;
...
class AuthController extends Controller
{
    public function authenticate(Request $request)
    {
        ...
        $binding = Binding::find($user->uid);
        $officialRoute = OfficialRouter::decide($binding);
        if ($binding && $officialRoute === 'p1') {
            try {
                $verified = MicrosoftAuth::verify($binding);
                Binding::updateTokens($user->uid, $verified['access_token'], $verified['refresh_token']);
                Log::channel('ygg')->info("Official route P1 verified for user [{$user->email}]");
            } catch (\Throwable $e) {
                Log::channel('ygg')->warning("Official route failed, using local fallback P2: {$e->getMessage()}");
                $officialRoute = 'p2';
            }
        }
        Log::channel('ygg')->info("Auth route [$officialRoute] selected for user [{$user->email}]");
    ...
    protected function getAvailableProfiles(User $user)
    {
        ...
        foreach ($user->players as $player) {
            $uuid = $binding && strtolower($binding->player_name) === strtolower($player->name)
                ? $binding->mojang_uuid
                : Profile::getUuidFromName($player->name);
