...
class ConfigController extends Controller
{
    public function render()
    {
        ...
        $officialForm = Option::form('official', trans('Yggdrasil::config.official.title'), function (OptionForm $form) {
            $form->checkbox('ygg_official_enabled', trans('Yggdrasil::config.official.enabled.title'))
                ->label(trans('Yggdrasil::config.official.enabled.label'));
            $form->select('ygg_official_auth_method', trans('Yggdrasil::config.official.auth_method.title'))
                ->option('device_code', trans('Yggdrasil::config.official.auth_method.device_code'))
                ->option('authorization_code', trans('Yggdrasil::config.official.auth_method.authorization_code'))
                ->hint(trans('Yggdrasil::config.official.auth_method.hint'));
            $form->text('ygg_official_device_client_id', trans('Yggdrasil::config.official.device_client_id.title'))
                ->hint(trans('Yggdrasil::config.official.device_client_id.hint'));
            $form->text('ygg_official_client_id', trans('Yggdrasil::config.official.client_id.title'))
                ->hint(trans('Yggdrasil::config.official.client_id.hint'));
            $form->text('ygg_official_client_secret', trans('Yggdrasil::config.official.client_secret.title'))
                ->hint(trans('Yggdrasil::config.official.client_secret.hint'));
            $form->text('ygg_official_redirect_uri', trans('Yggdrasil::config.official.redirect_uri.title'))
                ->hint(trans('Yggdrasil::config.official.redirect_uri.hint'));
            $form->text('ygg_official_timeout', trans('Yggdrasil::config.official.timeout.title'));
            $form->text('ygg_official_health_ttl', trans('Yggdrasil::config.official.health_ttl.title'));
        })->handle();

        Hook::addScriptFileToPage(plugin('yggdrasil-api')->assets('config.js'));

        return view('Yggdrasil::config', [
            'forms' => ['common' => $commonForm, 'keypair' => $keypairForm, 'official' => $officialForm],
        ]);
    }
